...
## 5. Regras de negócio prioritárias (mínimo 10)
...
Os diagramas devem ser versionados no repositório através do uso do plantuml e atualizados sempre que uma decisão de modelagem mudar.
...
**Exemplo de issue funcional:**

```
Título: [Feature] Confirmar agendamento de sessão

Descrição:
Como treinador, quero confirmar o agendamento de uma sessão para um aluno,
para garantir que a vaga seja reservada e a agenda atualizada.

Critérios de aceitação:
- [ ] O sistema recusa agendamento quando o horário está bloqueado (RN04).
- [ ] O sistema recusa agendamento quando a capacidade do horário foi atingida (RN06).
- [ ] O sistema recusa agendamento em caso de conflito de horário do aluno (RN05).
- [ ] Ao confirmar, o status da sessão muda para "Agendado".
- [ ] Testes automatizados cobrem os cenários de sucesso e recusa.

Regras de negócio afetadas: RN04, RN05, RN06

Tarefas técnicas:
- [ ] Implementar validação de bloqueio no serviço de agendamento.
- [ ] Implementar validação de capacidade e conflito.
- [ ] Criar endpoint POST /agendamentos.
- [ ] Escrever testes de unidade e integração.

Responsável: @nome-do-integrante
Rótulos: tipo:feature, area:api, area:dominio, prioridade:alta
```
